...
import { dateToString, computeTaskExecution, computeTaskDateStatus, daysOfInterval } from './utils/timeline-processor'

@Injectable()
export class TimelinesService {
    constructor(@InjectModel(Timeline.name) private timelineModel: Model<ITimeline>) {}

    async getHistoryProjects(projectId: string) {
        const timelines = await this.timelineModel.find({ project_id: projectId }).sort({ created_at: -1 })
    ...
    async getAll(projectId: string, permission: string = 'employee') {
        ...
        return timelines.map((timeline) => processTimelineToReturn(timeline, permission))
    }
    ...
    async getLatest(projectId: string) {
        const timeline = await this.timelineModel.findOne({ project_id: projectId }).sort({ created_at: -1 })
    ...
    async getDashboard(projectId: string) {
        ...
        const progressCurves: Record<string, { planned?: number; actual?: number; original?: number }> = {}
        ...
        for (const date of Object.keys(timeline.actual_progress ?? {})) {
            ...
            progressCurves[date].actual = timeline.actual_progress?.[date].progress_percentage
        }

        const firstTimeline = await this.timelineModel
            .findOne({
                project_id: projectId,
                _id: { $ne: timeline._id },
            })
        ...
        if (firstDay && lastDay) {
            const allDays = daysOfInterval(new Date(firstDay), new Date(lastDay), true, true).map(dateToString)
    ...
    async updateTimeline(projectId: string, timelineId: string, updateData: any) {
        // Update in MongoDB
        const updatedTimeline = await this.timelineModel.findOneAndUpdate({ _id: timelineId, project_id: projectId }, { $set: updateData }, { new: true })
        ...
        function deleteProgress(tasks: any[]): any[] {
            return tasks.map((task) => {
                if (task.actual_progress?.[dateToDelete] !== undefined) {
                    task.actual_progress = Object.fromEntries(Object.entries(task.actual_progress).filter(([key]) => key !== dateToDelete))
                }
    ...
    async updateTask(projectId: string, timelineId: string, taskId: string, updateData: any) {
        // Update in MongoDB
    ...
    async bulkUpdateTask(projectId: string, timelineId: string, updates: any[]) {
        ...
        const updatedTasks = updates.map((update) => updatedTimeline.tasks.find((task) => task.id === update.task_id))
